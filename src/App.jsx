...
function DataLandscape({ language, onCountrySelect, onLandscapeFocus, activeLandscapeFocus }) {
  ...
  const graph = useMemo(() => {
    ...
    const layoutPresets = {
      1: [{ x: 50, y: 62 }],
      2: [{ x: 25, y: 62 }, { x: 75, y: 62 }],
      3: [{ x: 18, y: 56 }, { x: 82, y: 56 }, { x: 50, y: 73 }],
      4: [{ x: 17, y: 55 }, { x: 83, y: 55 }, { x: 32, y: 72 }, { x: 68, y: 72 }],
      5: [{ x: 16, y: 54 }, { x: 84, y: 54 }, { x: 25, y: 69 }, { x: 75, y: 69 }, { x: 50, y: 78 }],
      6: [{ x: 15, y: 54 }, { x: 85, y: 54 }, { x: 25, y: 67 }, { x: 75, y: 67 }, { x: 39, y: 78 }, { x: 61, y: 78 }],
      7: [{ x: 15, y: 53 }, { x: 85, y: 53 }, { x: 23, y: 66 }, { x: 77, y: 66 }, { x: 36, y: 77 }, { x: 64, y: 77 }, { x: 50, y: 82 }],
      8: [{ x: 14, y: 53 }, { x: 86, y: 53 }, { x: 22, y: 65 }, { x: 78, y: 65 }, { x: 34, y: 76 }, { x: 66, y: 76 }, { x: 50, y: 82 }, { x: 50, y: 61 }],
    }
    const institutionPositions = activeGroup.institutions.map((_, index) => {
      ...
      const angle = ((360 / institutionCount) * index + 20) * (Math.PI / 180)
      return {
        x: 50 + Math.cos(angle) * 40,
        y: 66 + Math.sin(angle) * 22,
      }
    })
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-10 rounded-[28px] border border-black/10 bg-white/70 p-6 shadow-[0_12px_35px_rgba(0,0,0,0.05)]">
          ...
          >
            ...
            <div className="absolute bottom-4 left-4 max-w-[560px] rounded-2xl border border-black/10 bg-white/85 p-4 text-sm leading-6 text-neutral-600 shadow-sm">
              {selectedInstitutionNode ? (
                <>
                  ...
                  {language === 'en'
                    ...
                    : '이 열려 있습니다. 기관을 한 번 더 누르면 해당 기관의 자료 전체가, 개별 데이터 노드를 누르면 해당 자료만 아래 목록에 표시됩니다.'}
                </>
              ) : (
                language === 'en'
