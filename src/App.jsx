    ...
    <div className="relative overflow-hidden rounded-3xl border border-black/10 bg-[#0f2f24] p-6 text-white shadow-[0_12px_40px_rgba(0,0,0,0.14)]">
      ...
      <div className="relative">
        <div className="text-sm text-white/60">
          {language === 'en' ? 'Data map concept' : '아카이브 국가 지도'}
        </div>
        ...
        <p className="mt-3 max-w-md text-sm leading-6 text-white/70">
          {language === 'en'
            ...
            : '국가 카드를 선택하면 국가별 범죄 데이터를 빠르게 확인할 수 있습니다.'}
        </p>
...
function MetadataBar({ item, maxCount }) {
  ...
  return (
    <div>
      ...
      <div className="h-2 overflow-hidden rounded-full bg-black/10">
        <div className="h-full rounded-full bg-[#0f2f24]" style={{ width: `${width}%` }} />
      </div>
    </div>
  )
}
      ...
      <div className="mx-auto max-w-6xl">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-neutral-600 sm:text-base">
            {language === 'en'
              ? 'A quick metadata overview of the sources currently collected in this archive.'
              : '현재 아카이브에 수집된 데이터 출처를 메타데이터 기준으로 간단히 요약했습니다.'}
          </p>
        </div>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            value={totalDatasets}
            helper={language === 'en' ? 'Curated entries' : '선별된 항목 수'}
          />
          <StatCard
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border border-black/10 bg-white p-6">
            <h3 className="text-lg font-semibold tracking-tight">
              {language === 'en' ? 'Sources by country' : '국가별 출처 수'}
            </h3>
            <div className="mt-6 space-y-5">
              {countryStats.map((item) => (
                <MetadataBar key={item.name} item={item} maxCount={topCountryMax} />
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-black/10 bg-white p-6">
            <h3 className="text-lg font-semibold tracking-tight">
              {language === 'en' ? 'Top data types' : '주요 데이터 유형'}
            </h3>
            <div className="mt-6 space-y-5">
              {typeStats.map((item) => (
                <MetadataBar key={item.name} item={item} maxCount={topTypeMax} />
              ))}
            </div>
          </div>
        </div>
