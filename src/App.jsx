...
function DataLandscape({ language, onCountrySelect, onLandscapeFocus, activeLandscapeFocus }) {
  ...
  const graph = useMemo(() => {
    ...
    const layoutPresets = {
      // Tall oval around the active group node.
      // Top-side nodes use shorter links; lower-side nodes spread wider and slightly lower.
      1: [{ x: 50, y: 62 }],
      2: [{ x: 23, y: 42 }, { x: 77, y: 42 }],
      3: [{ x: 17, y: 35 }, { x: 83, y: 35 }, { x: 50, y: 70 }],
      4: [{ x: 18, y: 31 }, { x: 82, y: 29 }, { x: 30, y: 64 }, { x: 70, y: 66 }],
      5: [{ x: 19, y: 27 }, { x: 81, y: 27 }, { x: 15, y: 52 }, { x: 85, y: 52 }, { x: 50, y: 68 }],
      6: [{ x: 18, y: 26 }, { x: 82, y: 26 }, { x: 13, y: 48 }, { x: 87, y: 48 }, { x: 31, y: 70 }, { x: 69, y: 70 }],
      7: [{ x: 18, y: 25 }, { x: 82, y: 25 }, { x: 12, y: 45 }, { x: 88, y: 45 }, { x: 25, y: 67 }, { x: 75, y: 67 }, { x: 50, y: 70 }],
    ...
    const institutionPositions = activeGroup.institutions.map((_, index) => {
      ...
      // Fallback for countries with more than 8 institutions: distribute on a tall oval.
      const angle = (210 + ((120 / Math.max(institutionCount - 1, 1)) * index)) * (Math.PI / 180)
      return {
        x: 50 + Math.cos(angle) * 42,
        y: 30 - Math.sin(angle) * 46,
      }
    })
