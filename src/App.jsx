...
function DraggableNode({ node, position, language, isActive, onClick, onDragStart }) {
  ...
  const typeClass = {
    ...
    group: 'min-w-[132px] rounded-2xl border border-black/10 bg-[#f7f0e3] px-4 py-3 text-sm font-semibold text-[#0f2f24] shadow-sm',
    institution: 'min-w-[150px] max-w-[210px] rounded-2xl border border-black/10 bg-white px-4 py-3 text-[15px] font-bold leading-5 text-neutral-950 shadow-[0_8px_22px_rgba(0,0,0,0.06)]',
...
function DataLandscape({ language, onCountrySelect, onLandscapeFocus, activeLandscapeFocus }) {
  ...
  const graph = useMemo(() => {
    ...
    const institutionPositions = activeGroup.institutions.map((_, index) => {
      if (institutionCount === 1) return { x: 50, y: 55 }

      const angle = (-135 + (270 / Math.max(1, institutionCount - 1)) * index) * (Math.PI / 180)
      const radiusX = 34
      const radiusY = 22
      return {
              ...
              {landscape.groups.map((group, index) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl px-4 py-3 text-sm font-semibold transition ${isActive
                      ? 'bg-[#0f2f24] text-white hover:bg-[#143d2f]'
