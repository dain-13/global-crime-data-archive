...
function DataLandscape({ language, onCountrySelect, onLandscapeFocus, activeLandscapeFocus }) {
  ...
  const graph = useMemo(() => {
    ...
    const groupPositions = landscape.groups.length === 1
      ? [{ x: 50, y: 25 }]
      : landscape.groups.map((_, index) => ({
        x: index === 0 ? 29 : index === 1 ? 71 : 50,
        y: 25 + Math.floor(index / 2) * 8,
      }))

    landscape.groups.forEach((group, groupIndex) => {
      const groupNodeId = `group-${selectedCountry}-${groupIndex}`
      const groupPosition = groupPositions[groupIndex]
      const groupSourceIds = collectGroupIds(group)

      nodes.push({
        id: groupNodeId,
        type: 'group',
        label_en: group.title_en,
        label_ko: group.title_ko,
        x: groupPosition.x,
        y: groupPosition.y,
      ...
      group.institutions.forEach((institution, institutionIndex) => {
        const institutionNodeId = `institution-${selectedCountry}-${groupIndex}-${institutionIndex}`
        const isLeftGroup = groupIndex === 0
        const col = institutionIndex % 2
        const row = Math.floor(institutionIndex / 2)
        const x = isLeftGroup ? (col === 0 ? 16 : 39) : (col === 0 ? 61 : 84)
        const y = 43 + row * 10
        ...
        if (expandedInstitutionId === institutionNodeId) {
          ...
          institution.sources.forEach((source, sourceIndex) => {
            const sourceNodeId = `source-${selectedCountry}-${groupIndex}-${institutionIndex}-${sourceIndex}`
            const columns = Math.min(sourceCount, 4)
            const colIndex = sourceIndex % columns
            const rowIndex = Math.floor(sourceIndex / columns)
            const spacing = columns > 1 ? 70 / (columns - 1) : 0
            const sourceX = columns === 1 ? 50 : 15 + colIndex * spacing
            const sourceY = 84 + rowIndex * 9
    ...
    return { nodes, edges }
  }, [expandedInstitutionId, landscape, selectedCountry, selectedCountryMeta])
  ...
  const handleNodeClick = (event, node) => {
    ...
    if (node.type === 'institution') {
      setExpandedInstitutionId((current) => (current === node.id ? null : node.id))
      focusLandscape(node)
      return
    }
    ...
    <section id="landscape" className="border-b border-black/10 px-6 py-16 lg:px-8 lg:py-20">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-neutral-600 sm:text-base">
              {language === 'en'
                ? 'Drag nodes to rearrange the graph. Click an institution to reveal its connected datasets, then click a dataset to narrow the source list.'
                : '노드를 드래그해 배치를 바꿀 수 있습니다. 기관을 누르면 연결된 데이터가 나타나고, 개별 데이터를 누르면 아래 목록이 해당 자료로 좁혀집니다.'}
            </p>
          </div>
        ...
        <div className="mt-10 rounded-[28px] border border-black/10 bg-white/70 p-6 shadow-[0_12px_35px_rgba(0,0,0,0.05)]">
          ...
          <div
            ref={graphRef}
            className="relative mt-6 h-[680px] overflow-hidden rounded-[28px] border border-black/10 bg-[#f7f0e3] shadow-inner touch-none"
          >
            ...
            {!expandedInstitutionId && (
              <div className="absolute bottom-4 left-4 right-4 rounded-2xl border border-black/10 bg-white/80 p-4 text-sm leading-6 text-neutral-600 shadow-sm">
                {language === 'en'
