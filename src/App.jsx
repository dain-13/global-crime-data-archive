...
function DataLandscape({ language, onCountrySelect, onLandscapeFocus, activeLandscapeFocus }) {
  ...
  const isInstitutionActive = (institution) => {
    const institutionIds = institution.sources.map((source) => source.id).filter(Boolean)
    return activeLandscapeFocus?.type === 'institution'
  ...
  const isSourceActive = (source) => {
    return activeLandscapeFocus?.type === 'source'
      && activeLandscapeFocus.datasetIds.includes(source.id)
  }

  const NodePill = ({ children, active = false, muted = false, className = '', onClick, onMouseEnter, onMouseLeave, title }) => (
    <button
      type="button"
      title={title}
      onClick={onClick}
      ...
      className={`relative z-10 rounded-full border px-3 py-2 text-xs font-semibold shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${active
        ? 'border-[#0f2f24] bg-[#0f2f24] text-white'
        : muted
          ? 'border-black/10 bg-white/60 text-neutral-500 hover:border-black/20 hover:bg-white hover:text-neutral-800'
          : 'border-black/10 bg-white text-neutral-700 hover:border-[#0f2f24]/30 hover:bg-[#f7f0e3] hover:text-[#0f2f24]'} ${className}`}
    >
      {children}
    </button>
  )

  const GroupColumn = ({ group, side }) => (
    <div className="relative flex min-w-0 flex-1 flex-col gap-4">
      <div className="rounded-3xl border border-black/10 bg-[#f7f0e3] p-4 shadow-[0_8px_24px_rgba(0,0,0,0.04)]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="text-xs uppercase tracking-[0.16em] text-neutral-400">
              {side === 'left'
                ? language === 'en' ? 'Official' : '공식'
                : language === 'en' ? 'Research' : '연구'}
            </div>
            <h4 className="mt-1 text-lg font-semibold tracking-tight text-neutral-950">
      ...
      <div className="grid gap-3">
        {group.institutions.map((institution) => {
          const institutionActive = isInstitutionActive(institution)
          const isHovered = hoveredNode === institution.name_en
          return (
            <div
              key={institution.name_en}
              className={`relative rounded-3xl border p-4 transition ${institutionActive || isHovered
                ? 'border-[#0f2f24]/30 bg-white shadow-[0_10px_28px_rgba(15,47,36,0.10)]'
                : 'border-black/10 bg-white/75 hover:border-black/20 hover:bg-white'}`}
            >
              <div className={`pointer-events-none absolute top-1/2 hidden h-px w-10 bg-[#0f2f24]/20 lg:block ${side === 'left' ? '-right-10' : '-left-10'}`} />

              <div className="flex flex-col gap-3">
                <NodePill
                  active={institutionActive}
                  onClick={() => handleInstitutionClick(institution)}
                  onMouseEnter={() => setHoveredNode(institution.name_en)}
                  onMouseLeave={() => setHoveredNode(null)}
                  className="w-fit text-left"
                  title={language === 'en' ? 'Show all sources from this institution' : '이 기관의 데이터 모두 보기'}
                >
                ...
                <div className="flex flex-wrap gap-2 border-t border-black/10 pt-3">
                  {institution.sources.map((source) => (
                    <NodePill
                      ...
                      muted={!isSourceActive(source)}
                      onClick={() => handleSourceClick(source)}
                      title={language === 'en' ? 'Show only this dataset source' : '이 데이터만 보기'}
                    >
  ...
  return (
    <section id="landscape" className="border-b border-black/10 px-6 py-16 lg:px-8 lg:py-20">
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-4 inline-flex rounded-full border border-black/10 bg-white/70 px-3 py-1 text-xs font-medium uppercase tracking-[0.14em] text-neutral-500">
              {language === 'en' ? 'Institution graph' : '기관 연결망'}
            </div>
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">
              {language === 'en' ? 'Institutional data graph' : '기관별 데이터 연결 구조'}
            </h2>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-neutral-600 sm:text-base">
              {language === 'en'
                ? 'Explore each country as a lightweight knowledge graph: country, source type, institution, and dataset are connected as clickable nodes.'
                : '국가, 자료 구분, 기관, 데이터셋을 연결된 노드처럼 탐색할 수 있습니다. 기관을 누르면 관련 데이터가 모두 보이고, 개별 자료명을 누르면 그 자료만 표시됩니다.'}
            </p>
          </div>
        ...
        <div className="mt-10 overflow-hidden rounded-[28px] border border-black/10 bg-white/70 p-5 shadow-[0_12px_35px_rgba(0,0,0,0.05)] lg:p-6">
          ...
          <div className="relative mt-8">
            <div className="pointer-events-none absolute inset-x-0 top-[106px] hidden h-px bg-[#0f2f24]/10 lg:block" />
            <div className="pointer-events-none absolute left-1/2 top-[54px] hidden h-[105px] w-px -translate-x-1/2 bg-[#0f2f24]/10 lg:block" />

            <div className="mb-6 flex justify-center">
              <div className="relative z-10 rounded-[24px] border border-[#0f2f24]/20 bg-[#0f2f24] px-7 py-5 text-center text-white shadow-[0_12px_30px_rgba(15,47,36,0.20)]">
                <div className="text-xs uppercase tracking-[0.18em] text-white/55">
                  {language === 'en' ? 'Country node' : '국가 노드'}
                </div>
                <div className="mt-1 text-2xl font-semibold tracking-tight">
                  {language === 'en' ? selectedCountryMeta?.name_en : selectedCountryMeta?.name_ko}
                </div>
              </div>
            </div>
          ...
          <div className="mt-6 rounded-2xl border border-black/10 bg-[#f7f0e3] p-4 text-xs leading-5 text-neutral-500">
            {language === 'en'
              ? 'Tip: institution nodes filter multiple sources at once. Dataset nodes filter one source. This is a lightweight, responsive version of an Obsidian-style knowledge graph.'
              : '사용 팁: 기관 노드는 여러 자료를 한 번에 필터링하고, 데이터 노드는 하나의 자료만 필터링합니다. 옵시디언식 지식 그래프를 가볍고 반응형으로 구현한 형태입니다.'}
          </div>
        </div>
      </div>
    </section>
