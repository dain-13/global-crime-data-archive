...
function DraggableNode({ node, position, language, isActive, onClick, onDragStart }) {
  ...
  const typeClass = {
    ...
    group: 'min-w-[152px] rounded-2xl border border-[#0f2f24]/20 bg-[#f7f0e3] px-5 py-4 text-base font-bold text-[#0f2f24] shadow-[0_8px_22px_rgba(15,47,36,0.08)]',
    institution: 'min-w-[158px] max-w-[220px] rounded-2xl border border-black/10 bg-white px-4 py-3 text-[15px] font-extrabold leading-5 text-neutral-950 shadow-[0_8px_22px_rgba(0,0,0,0.07)]',
    source: 'min-w-[120px] max-w-[180px] rounded-full border border-black/10 bg-white/90 px-3 py-2 text-xs font-medium leading-4 text-neutral-700 shadow-sm',
  }[node.type]
...
function DataLandscape({ language, onCountrySelect, onLandscapeFocus, activeLandscapeFocus }) {
  ...
  const graph = useMemo(() => {
    ...
    const layoutPresets = {
      1: [{ x: 50, y: 61 }],
      2: [{ x: 30, y: 58 }, { x: 70, y: 58 }],
      3: [{ x: 20, y: 50 }, { x: 50, y: 66 }, { x: 80, y: 50 }],
      4: [{ x: 20, y: 47 }, { x: 80, y: 47 }, { x: 32, y: 68 }, { x: 68, y: 68 }],
      5: [{ x: 18, y: 45 }, { x: 82, y: 45 }, { x: 28, y: 66 }, { x: 50, y: 73 }, { x: 72, y: 66 }],
      6: [{ x: 18, y: 43 }, { x: 50, y: 31 }, { x: 82, y: 43 }, { x: 22, y: 67 }, { x: 50, y: 75 }, { x: 78, y: 67 }],
      7: [{ x: 16, y: 42 }, { x: 40, y: 31 }, { x: 64, y: 31 }, { x: 84, y: 42 }, { x: 25, y: 69 }, { x: 52, y: 76 }, { x: 78, y: 69 }],
      8: [{ x: 15, y: 41 }, { x: 37, y: 31 }, { x: 63, y: 31 }, { x: 85, y: 41 }, { x: 18, y: 68 }, { x: 39, y: 77 }, { x: 62, y: 77 }, { x: 82, y: 68 }],
    }
    ...
    <section id="landscape" className="border-b border-black/10 px-6 py-16 lg:px-8 lg:py-20">
      <div className="mx-auto max-w-6xl">
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-sm font-medium transition ${isActive
                    ? 'border-[#143d2f] bg-[#143d2f] text-white'
        ...
        <div className="mt-10 rounded-[28px] border border-black/10 bg-white/70 p-6 shadow-[0_12px_35px_rgba(0,0,0,0.05)]">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`rounded-xl px-4 py-3 text-sm font-semibold transition ${activeLandscapeButton === 'country'
                  ? 'bg-[#143d2f] text-white hover:bg-[#0f2f24]'
                  : 'border border-black/10 bg-white text-neutral-700 hover:border-black/20 hover:text-black'}`}
              >
          ...
          <div
            ref={graphRef}
            className="relative mt-6 h-[720px] overflow-hidden rounded-[28px] border border-black/10 bg-[#f7f0e3] shadow-inner touch-none"
          >
            ...
            <div className="absolute bottom-4 left-4 right-4 rounded-2xl border border-black/10 bg-white/85 p-4 text-sm leading-6 text-neutral-600 shadow-sm">
            ...
            >
              ...
              <div className="relative mx-auto max-w-4xl text-center">
                ...
                <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
                  <a
                    href="#filters"
                    className="min-w-[150px] rounded-md bg-[#143d2f] px-5 py-3 text-center text-sm font-medium text-white transition hover:bg-[#0f2f24]"
                  >
                ...
                {hasActiveFilters && (
                  <div className="mt-8 flex flex-col gap-3 rounded-2xl border border-black/10 bg-white/75 p-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button
                      type="button"
                      onClick={clearFilters}
                      className="rounded-xl bg-[#143d2f] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#0f2f24]"
                    >
