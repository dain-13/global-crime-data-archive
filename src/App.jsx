...
function splitNodeLabel(label, type) {
  const text = String(label || '')

  if (type === 'group') {
    if (text === 'Research and archive sources') return ['Research and', 'archive sources']
    if (text === 'Official government sources') return ['Official', 'government sources']
    if (text === 'Research and microdata sources') return ['Research and', 'microdata sources']
    if (text === 'Non-ministerial and research sources') return ['Non-ministerial', 'research sources']
    if (text === '비공식·연구자료') return ['비공식', '연구자료']
    if (text === '공식자료') return ['공식자료']
  }
  ...
  if (type === 'institution') {
    ...
    for (const separator of preferredBreaks) {
      if (text.includes(separator) && text.length > 16) {
        if (separator === ' and ') {
          return text.replace(' and ', ' and|').split('|').map((part) => part.trim()).filter(Boolean)
        }
        return text.split(separator).map((part, index, array) => {
          if (separator === '·' && index < array.length - 1) return `${part.trim()}·`
          return part.trim()
        }).filter(Boolean)
      }
    }
...
function DraggableNode({ node, position, language, isActive, onClick, onDragStart }) {
  const label = language === 'en' ? node.label_en : node.label_ko
  const labelLines = splitNodeLabel(label, node.type)

  const baseClass = 'absolute z-10 -translate-x-1/2 -translate-y-1/2 select-none text-center transition hover:-translate-y-[calc(50%+2px)]'
  const typeClass = {
    country: 'min-w-[150px] rounded-2xl border border-[#0f2f24] bg-[#0f2f24] px-5 py-3 text-base font-semibold text-white shadow-[0_10px_28px_rgba(15,47,36,0.22)]',
    group: 'min-w-[172px] max-w-[230px] rounded-2xl border border-[#0f2f24]/20 bg-[#f7f0e3] px-5 py-4 text-base font-bold leading-5 text-[#0f2f24] shadow-[0_8px_22px_rgba(15,47,36,0.08)]',
    institution: 'min-w-[168px] max-w-[240px] rounded-2xl border border-black/10 bg-white px-4 py-3 text-[15px] font-extrabold leading-5 text-neutral-950 shadow-[0_8px_22px_rgba(0,0,0,0.07)]',
    source: 'min-w-[124px] max-w-[210px] rounded-full border border-black/10 bg-white/90 px-3 py-2 text-xs font-medium leading-4 text-neutral-700 shadow-sm',
  }[node.type]

  const activeClass = isActive
    ? ' ring-2 ring-[#0f2f24] ring-offset-2 ring-offset-[#f7f0e3]'
    : ''

  return (
    <button
      type="button"
      onPointerDown={(event) => onDragStart(event, node, position)}
      onClick={(event) => onClick(event, node)}
      className={`${baseClass} ${typeClass}${activeClass}`}
      style={{ left: `${position.x}%`, top: `${position.y}%` }}
      title={label}
    >
      {node.type === 'institution' && (
        <span className="mb-1 block text-[10px] font-medium uppercase tracking-[0.14em] text-neutral-400">
          {language === 'en' ? 'Institution' : '기관'}
        </span>
      )}
      <span className="block whitespace-normal break-keep">
      ...
      {node.type === 'group' && (
        <span className="mt-1 block text-[11px] font-normal text-neutral-500">
          {node.count} {language === 'en' ? 'sources' : '개 자료'}
        </span>
      )}
    </button>
  )
...
function DataLandscape({ language, onCountrySelect, onLandscapeFocus, activeLandscapeFocus }) {
  ...
  const [activeLandscapeButton, setActiveLandscapeButton] = useState('group-0')
  const [expandedInstitutionId, setExpandedInstitutionId] = useState(null)
  ...
  const activeGroup = landscape.groups[activeGroupIndex] || landscape.groups[0]

  const collectSourceIds = (items = []) => items.map((source) => source.id).filter(Boolean)
  const collectInstitutionIds = (institution) => collectSourceIds(institution.sources)
  const collectGroupIds = (group) => group.institutions.flatMap((institution) => collectInstitutionIds(institution))

  const graph = useMemo(() => {
    const nodes = []
    const edges = []
    const countryNodeId = `country-${selectedCountry}`
    const groupNodeId = `group-${selectedCountry}-${activeGroupIndex}`
    const groupSourceIds = collectGroupIds(activeGroup)

    nodes.push({
      id: countryNodeId,
      type: 'country',
      label_en: selectedCountryMeta?.name_en || selectedCountry,
      label_ko: selectedCountryMeta?.name_ko || selectedCountry,
      x: 50,
      y: 8,
      datasetIds: landscape.groups.flatMap((group) => collectGroupIds(group)),
    })
    ...
    edges.push({ id: `edge-${countryNodeId}-${groupNodeId}`, source: countryNodeId, target: groupNodeId, type: 'main' })

    const institutionCount = activeGroup.institutions.length || 1
    // Institution nodes are arranged on a lower arc around the central source-type node.
    // Nodes closer to the top use shorter links; lower nodes spread wider so the graph reads like a loose circle.
    const layoutPresets = {
      // A vertically stretched ellipse: top-side nodes sit higher with shorter links,
      // while lower nodes spread wider left/right so the graph feels like an oval.
      1: [{ x: 50, y: 55 }],
      2: [{ x: 28, y: 49 }, { x: 72, y: 49 }],
      3: [{ x: 18, y: 36 }, { x: 50, y: 63 }, { x: 82, y: 36 }],
      4: [{ x: 16, y: 34 }, { x: 32, y: 58 }, { x: 68, y: 58 }, { x: 84, y: 34 }],
      5: [{ x: 14, y: 33 }, { x: 24, y: 49 }, { x: 50, y: 64 }, { x: 76, y: 49 }, { x: 86, y: 33 }],
      6: [{ x: 14, y: 32 }, { x: 22, y: 45 }, { x: 37, y: 60 }, { x: 63, y: 60 }, { x: 78, y: 45 }, { x: 86, y: 32 }],
      7: [{ x: 14, y: 31 }, { x: 21, y: 43 }, { x: 31, y: 55 }, { x: 50, y: 67 }, { x: 69, y: 55 }, { x: 79, y: 43 }, { x: 86, y: 31 }],
      8: [{ x: 14, y: 30 }, { x: 20, y: 40 }, { x: 28, y: 50 }, { x: 40, y: 62 }, { x: 60, y: 62 }, { x: 72, y: 50 }, { x: 80, y: 40 }, { x: 86, y: 30 }],
    }
    const institutionPositions = activeGroup.institutions.map((_, index) => {
      const preset = layoutPresets[Math.min(institutionCount, 8)]
      if (preset?.[index]) return preset[index]

      // Fallback for countries with more than 8 institutions: distribute on a vertically stretched ellipse.
      const angle = (205 + ((130 / Math.max(institutionCount - 1, 1)) * index)) * (Math.PI / 180)
      return {
        x: 50 + Math.cos(angle) * 36,
        y: 28 - Math.sin(angle) * 40,
      }
    })

    activeGroup.institutions.forEach((institution, institutionIndex) => {
      const institutionNodeId = `institution-${selectedCountry}-${activeGroupIndex}-${institutionIndex}`
      const position = institutionPositions[institutionIndex]
      const institutionIds = collectInstitutionIds(institution)
      ...
      nodes.push({
        ...
        x: position.x,
        y: position.y,
        datasetIds: institutionIds,
        countryValue: selectedCountry,
        institution,
      })
      edges.push({ id: `edge-${groupNodeId}-${institutionNodeId}`, source: groupNodeId, target: institutionNodeId, type: 'institution' })

      if (expandedInstitutionId === institutionNodeId) {
        const sourceCount = institution.sources.length || 1
        institution.sources.forEach((source, sourceIndex) => {
          const sourceNodeId = `source-${selectedCountry}-${activeGroupIndex}-${institutionIndex}-${sourceIndex}`
          const columns = Math.min(sourceCount, 5)
          const colIndex = sourceIndex % columns
          ...
          nodes.push({
            ...
            x: sourceX,
            y: sourceY,
            datasetIds: source.id ? [source.id] : [],
            countryValue: selectedCountry,
            source,
            parentInstitutionId: institutionNodeId,
          })
          edges.push({ id: `edge-${institutionNodeId}-${sourceNodeId}`, source: institutionNodeId, target: sourceNodeId, type: 'source' })
        })
      }
    })

    return { nodes, edges }
  }, [activeGroup, activeGroupIndex, expandedInstitutionId, landscape.groups, selectedCountry, selectedCountryMeta])

  const positions = useMemo(() => {
    return Object.fromEntries(
      graph.nodes.map((node) => [node.id, customPositions[node.id] || { x: node.x, y: node.y }]),
    )
  }, [customPositions, graph.nodes])

  useEffect(() => {
    setCustomPositions({})
    setExpandedInstitutionId(null)
    setActiveGroupIndex(0)
    setActiveLandscapeButton('group-0')
  }, [selectedCountry])
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event) => {
      ...
      const clampedX = Math.min(96, Math.max(4, nextX))
      const clampedY = Math.min(94, Math.max(6, nextY))
  ...
  const isNodeActive = (node) => {
    if (!activeLandscapeFocus?.datasetIds?.length || !node.datasetIds?.length) return false
    return node.datasetIds.some((id) => activeLandscapeFocus.datasetIds.includes(id))
  }

  const focusLandscape = (node) => {
    if (!node.datasetIds?.length) return

    onLandscapeFocus({
      datasetIds: node.datasetIds,
      label_en: node.label_en,
      label_ko: node.label_ko,
      countryValue: node.countryValue || selectedCountry,
      type: node.type,
    })
  }
  ...
  const selectedInstitutionNode = graph.nodes.find((node) => node.id === expandedInstitutionId)
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-4 inline-flex rounded-full border border-black/10 bg-white/70 px-3 py-1 text-xs font-medium uppercase tracking-[0.14em] text-neutral-500">
              {language === 'en' ? 'Institution graph' : '기관 그래프'}
            </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-sm font-medium transition ${isActive
                    ? 'border-[#0f2f24] bg-[#0f2f24] text-white active:bg-[#0f2f24] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f2f24]/35'
                    : 'border-black/10 bg-white/70 text-neutral-600 hover:border-black/20 hover:bg-white hover:text-[#0f2f24]'}`}
                >
        ...
        <div className="mt-10 rounded-[28px] border border-black/10 bg-white/70 p-6 shadow-[0_12px_35px_rgba(0,0,0,0.05)]">
          <div className="flex flex-col gap-4 border-b border-black/10 pb-6 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {landscape.groups.map((group, index) => {
                const isActive = index === activeGroupIndex
                return (
                  <button
                    key={`${group.title_en}-${index}`}
                    type="button"
                    onClick={() => {
                      setActiveGroupIndex(index)
                      setActiveLandscapeButton(`group-${index}`)
                    }}
                    className={`rounded-xl px-4 py-3 text-sm font-semibold transition ${activeLandscapeButton === `group-${index}`
                      ? 'bg-[#0f2f24] text-white hover:bg-[#143d2f] active:bg-[#0f2f24] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f2f24]/35'
                      : 'border border-black/10 bg-white text-neutral-700 hover:border-black/20 hover:text-[#0f2f24]'}`}
                  >
              ...
              <button
                type="button"
                onClick={() => {
                  setActiveLandscapeButton('country')
                  onCountrySelect(selectedCountry)
                }}
                className={`rounded-xl px-4 py-3 text-sm font-semibold transition ${activeLandscapeButton === 'country'
                  ? 'bg-[#0f2f24] text-white hover:bg-[#143d2f] active:bg-[#0f2f24] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f2f24]/35'
                  : 'border border-black/10 bg-white text-neutral-700 hover:border-black/20 hover:text-[#0f2f24]'}`}
              >
          ...
          <div
            ref={graphRef}
            className="relative mt-6 h-[780px] overflow-hidden rounded-[28px] border border-black/10 bg-[#f7f0e3] shadow-inner touch-none"
          >
            <div className="absolute inset-0 opacity-50" aria-hidden="true">
              <div
                ...
                style={{
                  backgroundImage:
                    'linear-gradient(to right, rgba(15,47,36,0.06) 1px, transparent 1px), linear-gradient(to bottom, rgba(15,47,36,0.06) 1px, transparent 1px)',
            ...
            <svg className="absolute inset-0 h-full w-full" aria-hidden="true">
              {graph.edges.map((edge) => {
                ...
                const isActive = activeLandscapeFocus?.datasetIds?.length
                  && graph.nodes.find((node) => node.id === target)?.datasetIds?.some((id) => activeLandscapeFocus.datasetIds.includes(id))
                ...
                return (
                  <line
                    ...
                    x2={`${target.x}%`}
                    y2={`${target.y}%`}
                    stroke={isActive ? '#0f2f24' : 'rgba(15,47,36,0.18)'}
                    strokeWidth={edge.type === 'source' ? 1.5 : 2}
                    strokeDasharray={edge.type === 'source' ? '4 6' : 'none'}
                  />
                )
              })}
            </svg>
            ...
            <div className="absolute bottom-4 left-4 right-4 rounded-2xl border border-black/10 bg-white/90 px-4 py-3 text-sm leading-5 text-neutral-600 shadow-sm">
              {selectedInstitutionNode ? (
                <p className="truncate">
                  <span className="font-semibold text-[#0f2f24]">
                    {language === 'en' ? selectedInstitutionNode.label_en : selectedInstitutionNode.label_ko}
                  </span>
                  {language === 'en'
                ...
                <p className="truncate">
                  {language === 'en'
                    ? 'Tip: click an institution node to reveal dataset nodes. Drag any node to rearrange the map.'
                    : '팁: 기관 노드를 누르면 연결된 데이터 노드가 나타납니다. 노드는 드래그해서 배치를 바꿀 수 있습니다.'}
                </p>
              )}
            </div>
          </div>
          ...
          <header className="sticky top-0 z-50 rounded-t-[28px] border-b border-black/10 bg-[#f7f0e3]/95 px-6 py-5 backdrop-blur lg:px-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <nav className="flex flex-wrap items-center gap-5 text-sm text-neutral-600">
                <a href="#about" className="transition hover:text-[#0f2f24]">
                  {language === 'en' ? 'About' : '소개'}
                </a>
                <a href="#filters" className="transition hover:text-[#0f2f24]">
                  {language === 'en' ? 'Search' : '검색'}
                </a>
                <a href="#browse" className="transition hover:text-[#0f2f24]">
                  {language === 'en' ? 'Countries' : '국가별 탐색'}
                </a>
                <a href="#landscape" className="transition hover:text-[#0f2f24]">
                  {language === 'en' ? 'Institutions' : '기관별 탐색'}
                </a>
                <a href="#snapshot" className="transition hover:text-[#0f2f24]">
                  {language === 'en' ? 'Snapshot' : '현황'}
                </a>
                <a href="#sources" className="transition hover:text-[#0f2f24]">
                  {language === 'en' ? 'Sources' : '데이터 출처'}
                </a>
                <button
                  onClick={() => setLanguage((current) => (current === 'en' ? 'ko' : 'en'))}
                  className="rounded-md border border-black/10 bg-white px-4 py-2 text-sm font-medium text-neutral-700 transition hover:border-black/20 hover:text-[#0f2f24]"
                >
                  {language === 'en' ? 'KR' : 'EN'}
                </button>
              </nav>
            ...
            >
              ...
              <div className="relative mx-auto max-w-4xl text-center">
                ...
                <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
                  <a
                    href="#filters"
                    className="min-w-[150px] rounded-md bg-[#0f2f24] px-5 py-3 text-center text-sm font-medium text-white transition hover:bg-[#143d2f] active:bg-[#0f2f24] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f2f24]/35"
                  >
                  ...
                  <a
                    href="#browse"
                    className="min-w-[150px] rounded-md border border-black/10 bg-white px-5 py-3 text-center text-sm font-medium text-neutral-700 transition hover:border-black/20 hover:text-[#0f2f24]"
                  >
                  ...
                  <a
                    href="#sources"
                    className="min-w-[150px] rounded-md border border-black/10 bg-white/70 px-5 py-3 text-center text-sm font-medium text-neutral-600 transition hover:border-black/20 hover:bg-white hover:text-[#0f2f24]"
                  >
                ...
                {hasActiveFilters && (
                  <div className="mt-8 flex flex-col gap-3 rounded-2xl border border-black/10 bg-white/75 p-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button
                      type="button"
                      onClick={clearFilters}
                      className="rounded-xl bg-[#0f2f24] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#143d2f] active:bg-[#0f2f24] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f2f24]/35"
                    >
                    ...
                    {hasMoreDatasets && (
                      <div className="mt-8 flex justify-center">
                        <button
                          ...
                          className="rounded-md border border-black/10 bg-white px-5 py-3 text-sm font-medium text-neutral-700 transition hover:border-black/20 hover:text-[#0f2f24]"
                        >
                          {showAll
                            ? language === 'en'
                              ? 'Show less'
